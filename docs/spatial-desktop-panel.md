...
`examples/spatial-desktop-panel` is the foreground interactive route. Termux, Termux:X11, XFCE, and x11vnc own the desktop and framebuffer. The Spatial SDK activity owns only panel placement, rendering of retained pixels, and translation of operator foreground input into bounded RFB events.
...
The app panel is 16:9 and spatially movable through the Spatial SDK
`Grabbable` component. The top title chrome is a visual grab affordance, not a
second Android-coordinate transform writer. Resize controls adjust meters
independently from desktop resolution. A single pure contain transform owns
desktop coordinate mapping, and the primary-pointer classifier separates taps,
