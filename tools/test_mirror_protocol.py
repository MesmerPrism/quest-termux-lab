...
def target_config(*, policy_kinds: list[str] | None = None, local_adb_enabled: bool = False) -> dict:
    return {
        "fleet_id": FLEET_ID,
        "agent_id": TARGET_ID,
        ...
        "operator_visible_session_active": True,
        "mirror_bindings": {
            SOURCE_ID: {
                "enabled": True,
...
class MirrorControllerTests(unittest.TestCase):
    ...
    def test_rejects_missing_lease(self) -> None:
        state = fleet_control_plane.FleetState()
        with self.assertRaisesRegex(ValueError, "mirror lease missing"):
            state.submit_mirror_intent(mirror_intent())
    ...
    def test_rejects_wrong_source(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "mirror source not allowed"):
            state.submit_mirror_intent(mirror_intent(source_agent_id="quest-agent-gamma"))

    def test_revoke_lease_rejects_new_intent(self) -> None:
        state = fleet_control_plane.FleetState()
        state.create_mirror_lease(mirror_lease())
        self.assertEqual(state.revoke_mirror_lease(LEASE_ID)["status"], "revoked")
        with self.assertRaisesRegex(ValueError, "mirror lease revoked"):
            state.submit_mirror_intent(mirror_intent())
