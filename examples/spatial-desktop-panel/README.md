...
While RFB is connected, the right Touch controller's A button emits one Linux secondary click at the current mapped desktop cursor position. Repeat key-down events are ignored, key down/up are consumed, and the normal keyboard letter A remains distinct. The on-panel **Right click** button remains as a visible fallback. This Android key-event route still requires real-controller acceptance on each relevant Horizon OS/Spatial SDK version; ADB-synthesized `KEYCODE_BUTTON_A` establishes only Android routing.
