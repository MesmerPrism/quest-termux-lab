...
class SpatialDesktopActivity : AppSystemActivity(), RfbListener {
  private lateinit var client: RfbClient
  ...
  override fun onSceneReady() {
    ...
    Log.i(
      ...
      "${SpatialPresentationContract.MARKER_ENTITY_CREATED} visible=${panelEntity?.getComponent<Visible>()?.isVisible} " +
        "transform=${panelEntity?.getComponent<Transform>()} ${panelComponentState()}",
    )
    drainPendingActions()
  }
  ...
  private fun installFirstDrawMarker(root: android.view.View) {
    ...
    listener =
      ViewTreeObserver.OnDrawListener {
        if (!logged) {
          logged = true
          Log.i(TAG, "${SpatialPresentationContract.MARKER_FIRST_DRAW} size=${root.width}x${root.height} visible=${root.visibility}")
  ...
  private fun bindPanelViews(root: android.view.View) {
    framebufferView =
  ...
  private fun isReady(action: PanelAction): Boolean =
    framebufferView != null && (action !is PanelAction.RecenterPanel || panelEntity != null)
      ...
      .onSuccess {
        ...
        Log.i(
          ...
          "${DebugPanelActionContract.MARKER_COMPLETED} requestId=${request.requestId} " +
            ...
            "forcedRelease=${framebufferView?.input?.forcedReleases ?: 0} scale=${"%.3f".format(physicalScale)} " +
            panelComponentState(),
        )
      }
      .onFailure {
  ...
  private fun dispatchPanelAction(action: PanelAction, source: String) {
    ...
    when (action) {
      ...
      PanelAction.SizeUp -> resizeBy(1.1f)
      PanelAction.SizeDown -> resizeBy(1f / 1.1f)
      PanelAction.RecenterPanel -> {
        require(BuildConfig.DEBUG) { "panel recenter disabled in release build" }
        recenterPanel(source)
      }
  ...
  private fun recenterPanel(source: String) {
    ...
    Log.i(
      ...
      "SPATIAL_DESKTOP_PANEL_RECENTERED finalPose=${placement.pose} source=${placement.source}:$source " +
        "scalePreserved=true ${panelComponentState()}",
    )
  }

  private fun panelComponentState(): String {
    val entity = panelEntity ?: return "grabbable=missing transform=missing scale=missing"
    val grabbable = entity.getComponent<Grabbable>()
    return "grabbable=present enabled=${grabbable.enabled} type=${grabbable.type} isGrabbed=${grabbable.isGrabbed} " +
  ...
  override fun onFramebuffer(frame: DecodedFrame, stats: RfbStats) {
    if (stats.frames == 1L || stats.frames % 60L == 0L) {
      Log.i(
        TAG,
        "SPATIAL_DESKTOP_RFB_FRAME fb=${frame.width}x${frame.height} generation=${stats.generation} " +
  ...
  override fun onStatus(message: String, stats: RfbStats) {
    Log.i(
      TAG,
      "SPATIAL_DESKTOP_RFB_STATUS status=${message.replace(' ', '_')} active=${client.isActive} " +
        "fb=${client.framebuffer.width}x${client.framebuffer.height} pixelFormat=${client.pixelFormat} " +
        "updates=${stats.updates} frames=${stats.frames} errors=${stats.errors}",
    )
    runOnUiThread {
  ...
  private fun refreshStatus() {
    ...
    statusView?.text =
      "${statusView?.tag ?: "ready"} fb=${client.framebuffer.width}x${client.framebuffer.height} " +
        "gen=${stats.generation} updates=${stats.updates} frames=${stats.frames} bytes=${stats.wireBytes} " +
        "socketMs=${stats.socketReadNanos / 1_000_000} decodeMs=${stats.decodeNanos / 1_000_000} " +
        ...
        "focusLoss=$focusLosses forcedRelease=${framebufferView?.input?.forcedReleases ?: 0} scale=${"%.2f".format(physicalScale)}\n$inputLine"
  }
